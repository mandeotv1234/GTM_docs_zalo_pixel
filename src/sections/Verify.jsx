...
const troubleshoot = [
  ...
  [<>/ptrck/events trả về <code>[]</code></>, 'Chưa tạo Conversion trong ZAM, hoặc URL không khớp Trang đích', 'Tạo Conversion trong ZAM. Dùng kiểu Có chứa nếu URL có tham số query.'],
  ['Click nút không thấy /ptrck/log', 'Quy tắc ZAM không khớp nút trên trang', 'Inspect nút bằng DevTools, kiểm tra id/class/text — phân biệt chữ hoa/thường.'],
  ['Beacon fire nhưng ZAM không hiện dữ liệu', 'Delay báo cáo thông thường', 'Chờ 24–48 giờ. Beacon 200 OK nghĩa là Zalo đã nhận — báo cáo có độ trễ xử lý.'],
  ...
  return (
    <section id="verify" className="pt-14">
      <SectionHeader
        ...
        desc="Sau khi publish GTM và tạo Conversion trong ZAM, dùng DevTools để xác nhận beacon đã gửi đến Zalo đúng cách."
      />
      ...
      <Step num={2} title="Reload trang — kiểm tra 2 request khởi tạo">
        ...
        <div className="space-y-2 mt-2">
          {[
            { url: 'GET /ptrck/events?pixelId=...&url=...', badge: '200 OK', badgeCls: 'bg-green-100 text-green-800', note: 'ztracker tải danh sách quy tắc conversion từ ZAM. Response là mảng JSON chứa các quy tắc id/class/text.' },
      ...
      <Step num={3} title="Nhấp vào nút đã cấu hình — kiểm tra beacon conversion">
        <p>Click vào đúng nút đã cấu hình trong ZAM. Phải xuất hiện ngay request <code>/ptrck/log</code>:</p>
        <div className="flex gap-3 items-start bg-white border border-slate-200 rounded-xl p-3.5 mt-2">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500 mt-1">
              ...
              <strong>eId</strong> — ID conversion trong ZAM &nbsp;·&nbsp;
        ...
        <Callout type="warn">Nếu click không ra /ptrck/log: Quy tắc trong ZAM không khớp. Inspect nút, kiểm tra lại id/class/text (phân biệt hoa/thường).</Callout>
      </Step>
